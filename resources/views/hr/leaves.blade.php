...
<div class="py-4">
    ...
    <!-- Stats -->
    @php
    $all       = $leaves + []; // to reset filter, use raw data passed
    $approved  = count(array_filter($leaves, fn($l)=>$l['status']==='Approved'));
    $pending   = count(array_filter($leaves, fn($l)=>str_starts_with($l['status'],'Pending')));
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl p-4 border border-gray-200 shadow-sm text-center">
            ...
            <p class="text-2xl font-bold text-gray-800 mt-1">{{ count($leaves) }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-gray-200 shadow-sm text-center">
            <p class="text-xs text-gray-500 uppercase font-medium">Disetujui</p>
            <p class="text-2xl font-bold text-green-600 mt-1">{{ $approved }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-gray-200 shadow-sm text-center">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-4 shadow-sm">
        <form method="GET" action="{{ route('hr.leaves') }}" class="flex flex-wrap gap-3 items-end">
            <div class="min-w-44">
            ...
            @if($status)
                <a href="{{ route('hr.leaves') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300">Reset</a>
            @endif
            <a href="{{ route('hr.leaves.create') }}" class="ml-auto px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h3 class="font-semibold text-gray-800">Daftar Pengajuan Cuti</h3>
            <span class="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{{ count($leaves) }} pengajuan</span>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3"><span class="px-2 py-1 bg-green-50 text-green-700 rounded-md text-xs font-medium">{{ $lv['type'] }}</span></td>
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ $lv['start'] }}</td>
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ $lv['end'] }}</td>
                        <td class="px-4 py-3 text-center font-bold text-gray-800">{{ $lv['days'] }}</td>
                        <td class="px-4 py-3 text-gray-600 max-w-xs truncate">{{ $lv['reason'] }}</td>
                        ...
                        @if(in_array(session('demo_user.role'),['admin','manager']))
                        <td class="px-4 py-3 text-center">
                            @if(str_starts_with($lv['status'],'Pending'))
                            <div class="flex gap-1 justify-center">
                                <form method="POST" action="{{ route('hr.leaves.approve', $lv['id']) }}" class="inline">
                                    @csrf
                    ...
                    @empty
                    <tr><td colspan="9" class="px-4 py-8 text-center text-gray-400">Tidak ada pengajuan cuti</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
